...
Set timeouts in application tool callbacks. The service has no total turn deadline and does not cancel a callback when the HTTP client disconnects; a stalled callback can hold a concurrency slot and a stateful session lease until it returns. Deployments with multiple replicas need a shared PostgreSQL session store, and per-process `disable` does not propagate to other replicas.
